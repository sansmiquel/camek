...
def _read_conf(p: pathlib.Path) -> dict:
        try:
            with open(p, 'r') as f:
                conf = json.load(f)
        except (IOError) as e:
...
def get_submodules_conf(conf=dict, submodules_list=list, conf_relpath=pathlib.Path) -> None:
    for submodule in submodules_list:
         p = conf_relpath.joinpath(submodule['configfile'])
         submodule_conf = get_conf(p)
         conf[submodule['id']] = {
              "conf": submodule_conf,
              "submodules_conf": {},
         }
         get_submodules_conf(
              conf=conf[submodule['id']]["submodules_conf"],
              submodules_list=conf[submodule['id']]["conf"]["_submodules"],
              conf_relpath=conf[submodule['id']]["conf"]["_conf_relpath"],
              )
    return None

def get_conf(p: pathlib.Path) -> dict:
        conf = _read_conf(p)
        conf['_conf_relpath'] = pathlib.Path(p.parent,*conf['_conf_relpath']).resolve()
        return conf
